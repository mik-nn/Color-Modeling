...
## 2026-07-09 — Статья #2: V3 (редакторская правка академической ветки) + LinkedIn-карусель + EN-версия

Три деривата статьи #2. (1) `docs/article2V2_en.html` — англоязычный порт V2 (терминология:
TAC, ink limit, chroma roll-off, ink holdout, media preset; аудитория — принтерные разработчики
и практики калибровки). (2) `docs/article2V3.html` — редакторская правка отдельной «академической»
ветки `article2V1_ru.html`: сохранены её ценные врезки для принтерных разработчиков (RGB-прокси
TAC / насыщение LUT 10-красочного P9000; потолок многокрасочных систем GCR/UCR), но исправлены
те же фактические ошибки, что в V2 (148/626 пар не патчей; фактические t=0.18/0.38/0.63/0.75,
пик t*=0.75; рис. 3 — реальные кривые C*(t); рис. 4 — масштабирование нижней половины тела,
~5 % не 20 %; retention «практически 100 %»). Врезка про holdout переписана: отделено
ИЗМЕРЕННОЕ (рост отражения, монотонность, decorr 0.03) от ГИПОТЕЗЫ (бронзинг/зеркалка/M0);
поправлена путаница «M1/M2 с поляризацией» → M2=UV-cut (OBA), поляризация=отдельная геометрия.
Академические заголовки разделов оставлены техничными, но убран лишний жаргон. (3)
`docs/substack/carousel/carousel2.html` — LinkedIn-карусель 7 слайдов в стиле первой
(`carousel.html`): обложка → «12 %» → хроматическая ошибка → механизм А (спад) → механизм Б
(holdout) → сенсор режима (+8 п.п.) → выводы+CTA. Рендер V3 (5 SVG) и всех 7 слайдов проверен
Playwright-скриншотами. Числа сверены с `data/h45_inklimit_gamut.json` и `h45c_control.json`.

Редакторская оценка иллюстраций (не перепроверка чисел, а «учит ли картинка идее»): две
слабые заменены на фигуры из РЕАЛЬНЫХ данных. (а) Рис. 4 был схемой блоба → построена
реальная оболочка Lab Signa 270 (905 патчей): вид сверху a*b* (контур до/после совпадает,
100 % насыщенности) + срез L*–C* (срезан только тёмный кончик, −4.5 % — это реальный dVpct
Signa из stage1; медиана когорты 5.4 %). (б) У механизма Б (§5) не было ни одной фигуры →
добавлен Рис. 5: среднее отражение нейтральной шкалы по TAC для DecorMatte vs впитывающего
ChromataWhite — кривые пересекаются ≈ TAC 145 (DecorMatte темнее на бумаге, светлее под
тяжёлой краской = holdout). Пасс-рейт стал Рис. 6. Тот же график невпитывания добавлен на
слайд 5 карусели (раньше — только карточки). Данные извлечены разовым дампом
(`buildCmyRamp`/`isOverLimit`/`spectraToLab`), временные скрипты удалены. Терминология —
строго из статьи (лимит краски, TAC, спад насыщенности, невпитывание, цветовой тон).

## 2026-07-08 — Статья #2: редакторская вычитка → docs/article2V2.html

Вычитал `article2_ru.html` против EXPERIMENTS.md и данных (`data/h45_inklimit_gamut.json`,
`data/h45c_control.json`); зелёные шкалы Signa270/DecorMatte воспроизвёл временным дампом
(tsx, buildCmyRamp) — значения C*ab совпали, но фактические покрытия патчей = 0/0.18/0.38/
0.63/0.75/1.0 (не номинальные 0.2-шаги), пик Signa t*=0.75. Правки: (1) «147 из 148 патчей»
→ пар (canon_fail_decomp: 148/626 пар); (2) Спирмен 0.71 и таблица покрытия атрибутированы
паре DecorMatte→ChromataWhite (H18) + добавлена общедатасетная фраза (H33: 1.1→3.3);
(3) заголовки таблиц шкал — фактические t, «последние 20 %» → четверть; (4) рис. 3
перегенерирован: вместо схемы a*b* — реальные кривые C*(t) обеих шкал (сдвиг оттенка зелёной
Signa всего ≈4°, схема преувеличивала); (5) потеря 5.4 % объёма — указан cohort (65 профилей,
чистые P9000 1.6–4.5 %), retention 100 % → «практически» (медиана 99.997);
(6) §6 — оговорка про «масочный» режим (KEY_FINDINGS #7: masked vs rescale). Рендер фигур
проверен Playwright-скриншотами. Итог: `docs/article2V2.html` (5 SVG валидны).

Вторая итерация по замечаниям автора — терминологическая: убраны самодельные термины
(«складка насыщенности» → спад насыщенности за точкой максимума / превышение лимита краски;
«покрытие краской» → подача краски / суммарное красконаложение (TAC); «перелив» → избыточная
подача; «оттенок» → цветовой тон; «holdout» → невпитывание чернил (ink holdout)); все
заголовки переписаны на общепринятый язык допечатной подготовки; добавлен абзац в §2,
объясняющий, откуда в RGB-наборе берутся C=255−R, M=255−G, Y=255−B и почему их сумма —
аналог TAC. Обновлены подписи и легенды всех SVG (рис. 1 — подписи в две строки,
рис. 3 — легенда укорочена), рендер перепроверен скриншотами.

Третья итерация: заголовок статьи → «Где пять патчей не срабатывают — и почему» (по плану
article2_plan.md, вариант 1); «Сколько цветового охвата стоит обрезка/ограничение» →
«Влияние лимита краски на цветовой охват»; рис. 4 перерисован по образцу типичных видов
цветового тела — вид сверху (a*b*, один контур) + вертикальный срез через ось L* (веретено
с остриём к чёрной точке; отсекаемый тёмный «колпачок» визуально ~5 %, а не ~20 % как в
старой схеме L*–C*); текст поправлен: в глубоких тенях высокой насыщенности не бывает,
тело сходится к чёрной точке остриём.

Четвёртая итерация (уточнение автора): ограничение CMY не «отсекает колпачок», а слегка
масштабирует нижнюю половину цветового тела — форма сохраняется, чёрная точка поднимается.
Рис. 4 (вид сбоку) перерисован: два подобных нижних контура (исходный штриховой и
ограниченный сплошной), сходящихся в самых широких точках тела; потерянные ~5 % — тонкий
слой между ними. Текст и подпись рисунка согласованы с этой моделью.

## 2026-06-26 — H45 предрегистрация: ink-limit ↔ объём цветового охвата

Новая гипотеза H45 (RESEARCH_HYPOTHESIS.md): на проблемных профилях вдоль рампы краски
chroma C*ab сначала растёт, потом падает (знак dC/dt меняется до полного покрытия) + hue
вращается на краю охвата. Ставим **внутренний** ink-limit на пике chroma (t* = argmax C*ab),
отсекаем overflow/holdout-патчи. Гипотеза: объём gamut почти не падает, а остаток становится
предсказуемым обоими предикторами. Объединяет H18 (ошибка ∝ покрытию, Spearman 0.712),
H35 (DecorMatte ink holdout) и H40 (hue rotation ±13–18° на краях).

Не круговая: limit берётся из собственной chroma-кривой профиля, НЕ из ошибки переноса;
H45d отдельно проверяет, совпадает ли граница с хвостом провала D1. Пороги:
H45a ΔV_hull ≤ 5% / reject ≥ 15%; H45b forward-LOO ΔE ↓≥0.5; H45c D1 pass-rate +≥10 pp;
H45d worst-5% recall ≥ 60%. Свип широкий — все ink-системы (P9000 BC + Canon G2470/G1430 +
MOAB SP7900/9900/P7000 + iPF4100/8100). DecorMatte обязан всплыть в проблемном cohort.

Разведка вскрыла: **CLAUDE.md/IMPLEMENTATION.md устарели** — `limitsAnalyzer/spectralPredictor/
cynsn/spreading/inkRatioAnalyzer` отсутствуют на диске (реальный код в `lib/predict/`).
Gamut volume и convex hull нет нигде; chroma/hue только в `lib/sampling/labSaturation.ts`.
Phase 2⁵ заведена в ROADMAP. Дальше: TDD `gamutVolume.ts` (hull + объём, тесты vs куб/тетраэдр).

## 2026-06-26 — Статья #2 черновик: «Where Five Patches Fail»

Написал продолжение статьи #1 (Predictive Color Adaptation on New Media): `docs/article2_en.md`
(~2360 слов) + RU-порт `docs/article2_ru.md` (~2030). Введение ссылается на статью #1. Ось:
провалы few-patch — хроматические на краю охвата (99% доминированы хромой), живут при высоком
покрытии (Spearman 0.71); два механизма — переинкинг (складка хромы, Signa270 green 82.8→70.4)
и holdout (DecorMatte монотонна 0→72.1, единственная нескладывающаяся). Главное: ошибка
предсказания = сенсор неподходящего режима; chroma-max t* = автоматический перцептивный ink-limit;
+8пп над random-контролем; обрезка стоит ~5% теневого объёма / 0% хромы. Лекарства — известный
print-prep (ink limit со ссылкой на «Pre-calibration of RGB Printers», правильный media-пресет);
открытая проблема (per-ink хроматический потолок) → тизер статьи #3. План: `article2_plan.md`.
Дальше: 5–6 фигур через make_figures.py + сборка в substack HTML.

## 2026-06-26 — H45c контроль: +7.5pp РЕАЛЬНЫЙ, не механический

Проверил, выживает ли +7.5pp при немеханической постановке (`h45c_control.ts`). Два фикса:
(1) предел определяется по REFERENCE-профилю (реалистично для деплоя), применяется по
device-координате; (2) matched-count random-контроль — выкидываем столько же СЛУЧАЙНЫХ
тест-патчей (среднее по 30 сидам). Результат на 230 cohort-парах: pass FULL 71.3% →
random-excluded **71.6%** (механика +0.3pp) → limit-excluded REF **79.6%**. **ΔREAL = +8.0pp**
над случайным базисом. Выкидывание стольких же случайных патчей не даёт ничего → прирост НЕ
артефакт урезания тест-сета; over-limit зона — реально там, где D1 падает, и работает даже с
пределом по REF. H45c повышен с «частично механический» до настоящего деплоируемого результата
(+8.0pp немеханических; всё ещё чуть ниже произвольного бара +10pp). KEY_FINDINGS #7 обновлён.

## 2026-06-26 — H45 результат: PARTIAL — ink-limit ≠ ink-holdout

Реализовал TDD 3 анализатора (`lib/analyzers/`): `gamutVolume.ts` (3D convex hull инкрементальный
+ объём через теорему дивергенции, тесты vs куб/тетраэдр/октаэдр), `inkLimitChroma.ts`
(детект t*=argmax C*ab по рампам в CMY-пространстве, DeviceSpace-agnostic), `forwardRampModel.ts`
(внутрипрофильный YN n forward-fit, ΔE residual). Все тесты зелёные (305/305 в полном прогоне).
Свип `h45_inklimit_gamut.ts` по 261 профилю всех 9 ink-систем → `data/h45_inklimit_gamut.json`.

Вердикт **PARTIAL**: H45b PASS (forward ΔE drop 0.656≥0.5), H45d PASS (worst-5% over-limit
recall 0.70≥0.60 — некруговое ядро: внутренний chroma-предел ловит 70% худших патчей переноса).
H45a: chroma-граница PASS (retention 100% — теряется только тёмный угол low-L*), но общий объём
маргинально мимо ≤5% (медиана 5.42%, раздут iPF8100 10.6%). H45c MISS (+7.5pp < +10, и частично
механический — урезание тест-сета по пределу убирает заведомо трудные патчи).

**Две предрегистрации ОПРОВЕРГНУТЫ**, и это главный научный итог: corr(signFlip,spreadCurv)=0.03
(независимые дескрипторы), и **DecorMatte (holdout-субстрат из H35) НЕ складывается** —
единственный из 24 P9000 без flip; chroma растёт монотонно до полной краски (Cyan 0→60.1,
Green 0→72.1, проверено отдельным дампом). Значит chroma-sign-flip — РЕАЛЬНЫЙ повсеместный
механизм (23/24 P9000), безопасно срезает ~4–5% тёмного объёма при 100% сохранении цветовой
границы, НО это ДРУГОЙ механизм, чем ink-holdout DecorMatte/H40. Интуиция пользователя про
сохранение охвата подтвердилась (цветовая граница цела); но «проблемные» holdout-профили — не
chroma-fold, их лечит per-ink хроматическая модель (KEY_FINDINGS #5), а не ink-limit. Зафиксировано
в KEY_FINDINGS #7. Замечание: CLAUDE.md/IMPLEMENTATION.md перечисляют удалённые анализаторы —
добавил 3 новых, старые помечены как отсутствующие.
...
12-уровневый куб {0, 23.18, ..., 255}=255/11, white L*100, black L*17, cyan L*64). Реальный баг:
P9900 каталог мешает ТРИ device-сетки — 905 (ZXML M0, 22 файла), 1728 (CIED+DevD M2/UVcut, 13),
774/400/674. Я парил профили РАЗНЫХ сеток. alignProfiles по device-координатам: 1728↔1728 = 1728
exact, но **905↔1728 = только 12 exact** (совпадают лишь на 0/255), остальные интерполированы
k-NN → выдуманный ground truth → ложные провалы. P9900 within-grid: 1728-группа 11%→34.6%.

Фикс: guard `patch_count` равенства в h44_anchor_stability/profile_count_sweep/cross_printer_transfer.
Новое HARD-правило (KEY_FINDINGS §2, AGENTS.md, память): парить только в одной сетке + условии
измерения (M0/M2). Перезапуск transfer: P9900 native 41%→57%. Epson HDX перенос асимметричен
(P9900-chart→P9000 95%, P9000-chart→P9900 39%) — конфаунд M0(P9000) vs M2/UVcut(P9900) + разные
сетки, не различие чернил. Canon dye остаётся чистым подтверждением per-ink-system (73/64≈native).
...
принтера загружаем все профили, запускаем `pickCoverageAnchors`, проверяем отклонение
device-RGB координат якорей. Результат: 5/6 принтеров INVARIANT (отклонение = 0) —
координаты COV6 якорей substrate-invariant для CxF-профилей. P9900 и iPF8100-BC (формат
CIED+DevD) показывают 12–18 device-unit отклонение — предположительно артефакт
парсинга разных chart layout'ов, а не реальная нестабильность. Вывод: для выбора
координат нужен 0 существующих профилей.

Part B (H44-D): COV6 same-mode pass-rate на 100 парах per printer: G2470 18%, G1430 15%,
P9000 66%, P9900 9%, iPF4100 9%, iPF8100-BC 15%. P9000 66% согласуется с ранее ~80%
(без металликов). Для Canon G2470/G1430 низкий % (15–18%) объясняется высоким
разнообразием субстратов внутри принтера (all-pairs vs same-family). Гипотеза B
(сложность чернил → сложность якорей) не подтверждена.

Разархивировано iPF4100: `data/profiles/ipf-pro-4100/extracted/` (23 профиля из 23 zip).

## 2026-06-20 — H44 framing correction

**Обнаружена методологическая ошибка в H44-A и H44-B:** оба эксперимента оценивали H4 gate
(med≤1.5, P95≤3.0) на кросс-режимных парах (любой режим → любой режим). H4 gate определён
только для пар одного режима (same-mode). Поэтому результаты 0–22% измеряют сложность
кросс-режимного переноса, а не целевой вопрос H44.

**Верный вопрос H44:** сколько измеренных профилей конкретного принтера/красочной схемы
нужно, чтобы НАДЁЖНО ВЫБРАТЬ координаты опорных точек (RGB device values), которые дадут
оптимальный D1 на парах одного режима? Это вопрос о стабильности выбора якорей (dataset-count
question), а не о геометрии патч-чарта.

**Что остаётся валидным:** инфраструктура парсеров для 7 принтеров (iPF8100 CIED+DevD 1728
патчей, iPF4100 после unzip, Canon G2470, P9900, MOAB targ), h44_manifest.json 240 профилей.
Кросс-режимный результат (90% same-mode → 7–22% cross-mode) реален и важен, но относится
к отдельной гипотезе (H45?).

**Следующие корректные эксперименты H44:** H44-C (стабильность RGB якорей COV5 при n=1→27
профилей, P9000 same-mode), H44-D (разрыв H4 pass-rate при n=1 vs pool), H44-E
(дисперсия якорей vs сложность красок: G2470 4-ink vs iPF4100 12-ink).

Скорректированы: `docs/RESEARCH_HYPOTHESIS.md` (H44 section переписан), `docs/EXPERIMENTS.md`
(строка-корректировка добавлена наверх append-only лога).
...
Обсуждение перед сборкой генератора: насколько 6 патчей хуже 12 и кого именно теряем.

**H41 — кто fail@cov6, pass@S1-12.** cov6 валит 24/104 = **12 FLIP** (S1-12 спасает
ровно их) + 12 FAIL (структурные). FLIP-сигнатура: медиана@cov6 в норме (1.24–1.60),
падают ТОЛЬКО на P95 (3.0–5.4) из-за тяжёлой краски — heavy-tercile ΔE@cov6=3.07 (vs
ALWAYS 1.53) → 1.54 на S1-12. dCurv разделяет классы: FLIP=0.081 (низкий, spreading-
совместимы), FAIL=0.147 (≈порог). FLIP-субстраты: PremiumGlossy/CanvasSatin/Watercolor
(high-Dmax). **Why:** разрыв 76.9→88.5 — это heavy-ink P95-хвост на совместимых парах;
одинокий black+gray128 в cov6 недосэмплирует full-CMY интерьер → экстраполяция. Флаг
spreadCurv (считается на самом cov6) предсказывает класс: низкий dCurv = поможет больше
патчей, высокий = структурный потолок.

**H42 — дешёвый фикс: +2 нейтрали vs secondaries.** cov8n (cov6+gray64+gray192) =
**82.7%** (+5.8pp), забирает 6/12 FLIP. cov8s (+red+green)=78.8%, 2/12. cov9 (+R+G+B)=
81.7%, 5/12. S1-12=88.5%, 12/12. **2 нейтрали бьют 3 secondaries** (82.7 vs 81.7) при
меньшем числе патчей. **Why:** FLIP-восстановление — это coverage-ось краски, сэмплится
нейтральной рампой, не gamut-углами. Лестница для UI: **cov6=77% / cov8n=82.7% / S1-12=
88.5%**. Применить в плане генератора (substrate-dataset-generator.md): чарт 6/8/12
выбираемый; cov8n — sweet spot.

См. EXPERIMENTS.md H41/H42 (2026-06-17).

---

## 2026-06-16 — H40: анатомия хвостов провальных пар (вопросы пользователя)

`h40_failtail_anatomy.ts`, 12 провальных пар. Ответы:

**(A) Сколько правильно:** агрегат 61.6% патчей ΔE≤2, **79.6% ΔE≤3**, только 5.3% >5. Худшая пара всё равно 67% ≤3. Провальная пара ≠ битый профиль — провал это узкий хвост ~20%.

**(B) Как выглядят хвосты:** chroma bias со СМЕНОЙ ЗНАКА по направлению. X→DecorMatte: ΔC −4.7…−7.2 (предсказание перенасыщено, desat 95-100%); DecorMatte→X: ΔC +5.7…+9.2. Worst-10 = hue-shift на хроматических экстремумах: оранж (255,~140,0) ±13-14°, тёмно-зелёный (0,85,0) −18ΔC +17°.

**(C) Постеризация — НЕТ:** градиент-ratio 0.84–1.23 (≈1, градации целы).
**(D) Соляризация — НЕТ:** инверсий L* ≈0.
**(E) Соседская когерентность:** 98-99% плохих патчей выровнены с соседом → гладкий локальный bias, не изолированные скачки/шум.

**Вывод:** 12 «битых» датасетов на 80% предсказаны верно (ΔE≤3); провал = узкий КОГЕРЕНТНЫЙ хвост, не шум, не артефакты. Подозреваемые постеризация/соляризация ОТСУТСТВУЮТ. Хвост = гладкий направленный хроматический bias (chroma collapse/inflate + hue rotation), знак задан тем DecorMatte источник или цель. Когерентен → структурно корректируем, но ХРОМАТИЧЕСКИ — почему нейтральный spreading (H38) не справился. См. EXPERIMENTS H40.
...
- **Корреляция:** r(Δcurv560,p95)=0.48, broadband 0.52; mean Δcurv fail=0.147 vs pass=0.062 (2.4×).
- **ROC:** AUC=0.841; порог Δcurv560≥0.137 → 10/12 провалов, 6/92 false-alarm (Youden J=0.77).
- **Reference selection (главное):** ближайший по spreadCurv ref bьёт дальний 10/14; mean p95 1.52 vs 2.96 (вдвое); pass-rate 13/14 vs 9/14.
- **Кластеризация:** spreadCurv сортирует подложки по режиму печати (CanvasMatte 1.58–1.82, WCRW 1.98–2.14, Luster/Satin/Glossy 2.25–2.43). DecorMatte 1.577 выброс.

spreadCurv = первый интерпретируемый физический параметр в проекте, валидированный как предиктивный. Измеряется из ~8 нейтральных патчей, только M0 (иммунен к артефакту M2@380). Два продакшн-применения: (1) pre-flight флаг провала; (2) рекомендатель reference-профиля. Физика: low=ink holdout/on-surface (matte canvas), high=absorbed (glossy); пары через раздел holdout/absorb ломают нейтрально-якорный трансфер (H35). См. RESEARCH_HYPOTHESIS H36, EXPERIMENTS.md H36.

**Также:** добавил caveat в H34 — «UV-absorber» sub-claim опирается на M2@380 (известный extrapolated-fallback артефакт); главный вывод H34 (OBA не причина) не затронут.
...
## 2026-06-16 — H33: анатомия провалов — какие патчи / длины волн / подложки

`scripts/experiments/h33_failure_anatomy.ts`. 3 уровня на 104 неметаллических парах при k=13.

**(P) КАКИЕ ПАТЧИ:** ошибка растёт с инком — light 1.13 → C+M+Y-dark 3.32 mean (p95 6.68 vs passer 2.32). Худшие fail-excess секторы: тёмный (+2.20), жёлтый (+1.37), красный M+Y (+1.26). Мисс = тяжёлый инк, особенно полный CMY и жёлто-красная сторона.

**(W) КАКИЕ ДЛИНЫ ВОЛН:** failers расходятся 2.0–2.8× во всех полосах; пик ratio 500nm (2.8×), пик абсолют 380nm (UV/OBA). Два режима: 1930↔ArtPeel — λ=540–580 (жёлтая краска); VibranceLuster↔RiverStone — λ=380–410 (UV/OBA) + 690.

**(S) КАКАЯ ПОДЛОЖКА:** ТОЛЬКО spreading-кривизна нейтральной рампы коррелирует с частотой провала (r=−0.64); b* (0.04), кривизна бумаги (0.07), гамут (−0.15) — НЕТ. DecorMatte = выброс с низкой spreadCurv (1.58) внутри CanvasMatte (пиры 1.7–1.8).

**Единая анатомия:** мисс-патчи = тяжёлый инк (модель экстраполирует нелинейное ink-взаимодействие в закрашенную внутренность от paper+corners якорей); причина = mismatch spreading-кривизны, остаток ХРОМАТИЧЕСКИЙ (по краске, жёлтая 540–580) + UV/OBA на тёмных; виновата подложка-spreading-ВЫБРОС внутри режима, не сам режим (провал ПАРНЫЙ — все CanvasMatte-провалы вокруг DecorMatte). Не предсказывается белизной/желтизной/кривизной бумаги/гамутом. Даёт a-priori предиктор провала: большой |ΔspreadCurv| (H23a) + остаток в одной полосе поглощения краски. См. `EXPERIMENTS.md` H33.

**Next:** pre-flight флаг провала в UI (spreadCurv-выброс → предупреждение ~11.5% структурного провала). Реальный фикс = per-ink хроматический spreading, не тянем (убывающая отдача).
...
**Результат:** S1 k=5=45.2%, S1 k=6=49.0%, **H31 coverage k=6=76.9%**, S1 k=13=88.5%. **H31a PASS** (76.9%≥75%, +31.7pp над S1 k=5). **H31b PASS** (12/12 стойких всё ещё fail — структурные).

**Placement изолирован:** при одинаковом k=6 coverage +34 пары / −5 / net **+29** vs S1 k=6. +27.9pp ТОЛЬКО от того, где сидят 6 якорей. Доказано: low-k провал D1 = размещение по оси инк-покрытия, не количество. CMYKOG-интуиция юзера подтверждена для RGB/CMY.

**«D1 нужно 13» → «D1 нужно coverage-spanning 6».** 28 из ~43pp low-k дефицита — placement; остаток 11.6pp (76.9→88.5) — count/плотность. 12 структурных провалов переживают хорошее размещение (независимый потолок).

**Практика:** фиксированный 6-патч чарт {white,C,M,Y,black,gray} — без per-pair эвристики и без target-профиля, ~77% pass. Развёртываемый few-patch D1 протокол (нейро H22 k=5=84.6% всё ещё выше по pass, но D1-чарт target-agnostic). См. `EXPERIMENTS.md`/`RESEARCH_HYPOTHESIS.md` H31.

---

## 2026-06-16 — Диагностика: где D1 ошибается при малом k + анатомия 12 стойких провалов

`scripts/experiments/d1_lowk_patch_analysis.ts`. Per-patch ΔE при k=5/13, бины по инк-покрытию и расстоянию до ближайшего якоря; классификация пар.

**Q1 — ошибка при k=5 определяется ИНК-ПОКРЫТИЕМ, не расстоянием до якоря.** r(ΔE,ink)=**0.606** >> r(ΔE,nnDist)=0.363. Тяжёлый инк [2.5,3.0]: mean ΔE=5.01 (low-k-fail) vs 2.48 (always-pass); низкий инк ≈0.9 у всех. S1 k=5 = {paper(0), red(2), green(2), blue(2), cyan(1)} — **нет якоря при ink=3**, только 1 primary → тяжёлый угол гаммы экстраполируется = там и взрыв. Переводит CMYKOG-интуицию юзера (6 красок ×{100,50%}+бумага=13) на RGB/CMY: D1-остаток нужно сэмплить на нескольких уровнях покрытия по каждому направлению.

**Q2 — 12 стойких (11.5%) = 3 механизма:** (1) DecorMatte 8/12 (CanvasMatte, матовая некрытая vs крытые; но DecorMatte↔800M fail при ΔE_ab=0.9 → глубже paper-white); (2) VibranceLuster↔RiverStone 2 (ΔE_ab=7.5); (3) 1930↔ArtPeelBlckt 2 (ΔE_ab=1.1, основа почти идентична — чистый spreading, H23a). **Paper-white ΔE_ab НЕ разделяет** fail(med 5.45)/pass(med 4.50, max 8.83>failer). Дискриминатор — спектральная форма/spreading, не величина разницы основ.

**Гипотеза H31 (зарегистрирована):** coverage-spanning k≈6 якорей {paper + 3 primaries + full CMY(ink3) + mid-neutral}. Прогноз: D1 прыгнет с 45% (S1 k=5) к ~k=13 (88.5%) на ~6 якорях — экстраполяция→интерполяция на тяжёлом инке. Стойкие 12 не затронет (структурные). См. `EXPERIMENTS.md`/`RESEARCH_HYPOTHESIS.md` H31.

---

## 2026-06-16 — H30: low-k переобучение attention — 3-патч протокол 82.7% (H30b PASS, H30a FAIL)

`scripts/h30_train.ts` = H29 с k-aug {3,4,5} вместо {5,8,13}; eval k∈{3,4,5,8}. Архитектура та же (τ + hard top-k′=4 + residual).

**Результат:** k=3=82.7% (med 0.714), k=4=82.7%, k=5=82.7%, k=8=80.8%. τ=0.40. **H30b PASS** (k=3≥70% → рабочий 3-патч протокол, D1 k=5=45%). **H30a FAIL** (k=5=82.7% < H22 84.6%). Low-k обучение → почти k-инвариантность на 3–5 (все 82.7%). H27 D1-only пары 0/3 на k=5.

**Вывод:** H30 даёт нейро-протокол на 3 патчах (82.7%), где D1 непригоден, но НЕ бьёт H22 mean-pool на k=5. Ценность — плавное расширение вниз до k=3 (in-distribution для H30, OOD для H22), не более высокий потолок на k=5.

**Консолидированная маршрутизация (лестница по бюджету якорей):** k=3-4 → H30 (82.7%); k=5 → H22 (84.6%); k=6-8 → H29 (85.6%); k≥10 → D1 (88.5%). Нейро-семейство = лестница для few-patch адаптации, не один убийца D1. См. `EXPERIMENTS.md`/`RESEARCH_HYPOTHESIS.md` H30.

---

## 2026-06-16 — D1 anchor-sweep: при k=5 D1 рушится до 45% — переворачивает вывод «шипить D1»

Замерил D1 на 104 неметаллических парах при k∈{5,8,13} (`scripts/experiments/d1_anchor_sweep.ts`) — раньше был залогирован только k=13. **D1 k=5=45.2%, k=8=82.7%, k=13=88.5%.** Против нейро: H22 k=5=84.6%, H29 k=8=**85.6%**.

**Вывод (переворот):** D1 выигрывает ТОЛЬКО при k=13. При k=5 D1 непригоден (45%) — paper-ratio + rank-5 residual + IDW(k=4) недоопределены на 5 точках. При k=8 H29 (85.6%) БЬЁТ D1 (82.7%). Кроссовер ≈ k=8. Цель проекта (few-patch адаптация) живёт именно там, где нейро побеждает. Прежний вывод H29 «закрыть нейро-линию, шипить D1» **отозван** — нейро не «хуже D1», а обслуживает low-anchor режим, который D1 не может.

**Маршрутизация по бюджету якорей: нейро (H29) для k≤8, D1 для k≥~10.** Open: переобучить H29 с k-aug {3,4,5} — побьёт ли 5-патч число H22 84.6%? См. `EXPERIMENTS.md`/`RESEARCH_HYPOTHESIS.md`.

---

## 2026-06-16 — H29: заострённое device-attention (τ + hard top-k′) — FAIL ворот, но лучший нейро-результат

`scripts/h29_train.ts` = H28 + два изменения: (1) обучаемая температура τ=exp(logτ) на скорах attention; (2) hard top-k′=4 device-gating перед softmax (маскируем всё кроме 4 ближайших по CMY якорей — буквальный обучаемый IDW = D1 knnK).

**Результат (104 неметаллические):** k=5=82.7%, **k=8=85.6% (лучший нейро-результат за все эксперименты, бьёт H22 83.7%/84.6% и H28)**, k=13=84.6%. **Обученная τ=0.413<1** — сеть сама выбрала заострение, что подтверждает диагноз H28 о размытости; P95 упал 2.84(H28)→2.41. H29b: 1/3 D1-only восстановлено (PhotoPeelGloss→VibranceGloss 4.83→3.04 почти прошла; обратное направление VibranceGloss→PhotoPeelGloss РЕГРЕССировало 3.44→4.91).

**Вывод:** заострение — правильная ось (τ самонастроилась на 0.41), но семейство learned-attention асимптотирует ~85–86% < D1 88.5%. Обучаемое device-ядро над *дельтами* якорей помогает одним направлениям, вредит другим (асимметричная дисперсия, которой нет у детерминированного IDW D1). **Линия H22→H28→H29 закрыта: все три ниже D1.** D1 k=13 (88.5%) = production-модель; ценность нейропути — эффективность при малом k (H22, 5 патчей ~80%), не пиковая точность. См. `EXPERIMENTS.md`/`RESEARCH_HYPOTHESIS.md` H29.

**Рекомендация:** прекратить тюнинг device-delta-attention (убывающая отдача vs D1). Если возвращаться — менять представление: attention над полными *спектрами* якорей (не дельтами), graph/message-passing над device-решёткой, или гибрид (D1 IDW база + нейро-остаток только на провалах D1). Иначе принять 88.5% (D1) как потолок локальности датасета и шипить D1.
...
**Результат:** D1 FULL=92/104=88.5% → noL3=79/104=76.0%. **Layer 3 стоит +12.5 pp**, 13 пар отваливаются, 0 новых проходов. Все 3 D1-only пары из H26 (Crystalline×2, PhotoPeelGloss↔VibranceGloss) рушатся без Layer 3. Δp95 (noL3−full) ≥ 0 везде (median +0.42, max +3.85) — локальная коррекция никогда не вредит.

**Why:** H26 оставил гипотезу, что преимущество D1 над H22 = именно Layer 3. H27 подтверждает количественно: без Layer 3 D1 падает до 76.0% — НИЖЕ H22 k=5 (84.6%). Глобальный paper-ratio + PCA-базис недостаточны; локальная device-space-адаптивность несёт маргинальные пары. Обосновывает H28: перенести локальность в нейросеть через per-anchor attention. См. `EXPERIMENTS.md` строку H27 (2026-06-16).

---

## 2026-06-15 — H26: H22 anchor-composition analysis — нейронная сеть не захватывает нелинейность, которую пропускает D1

**H26 FAIL (обе ворота)**. D1 k=13=88.5% > H22 k=5 S1=84.6%. Перестановка якорей на CMY-первичные (paper+C+M+Y+black) → 0/104 — катастрофический OOD-отказ. Cross-tab: H22-only=0 пар (нейронная сеть не уникальна ни для одной пары), D1-only=4 (Crystalline, PhotoPeelGloss↔VibranceGloss; D1 выигрывает за счёт IDW-коррекции в Layer 3). H22 запоминает конкретный состав якорного набора ({paper,R,G,B,cyan}), не обобщается. k=8 незначительно хуже k=5 (87 vs 88) — согласуется с тем же эффектом. **Вывод: mean_delta-архитектура = глобальная модель, не локальная; D1 Layer 3 IDW = локальная нелинейная коррекция, именно она даёт преимущество D1.**

---

## 2026-06-15 — H23/H24: spreading-механизм подтверждён, коррекция по нейтральному рампу исследована

**H23a PASS** (AUC=0.848): ΔSpread (квадратичная кривая нейтрального рампа при λ=560nm)
является статистически значимым бинарным предиктором отказа H4 на нечёрно-белых парах.
TPR=0.833, FPR=0.087. Опровергает вывод H22 о «структурной несовместимости» как основной причине — реальная причина: разница dot-gain/spreading между субстратами.

**H23b FAIL**: скалярная коррекция `s(a)` при λ=560nm, применяемая ко всем 36 полосам —
неверная модель. 5 регрессий (oracle вариант), acceptance gate не пройден. Причина: dot-gain
нелинейно специфичен для каждой длины волны; OBA (λ<450nm) имеет другой знак/форму.

**H24 PARTIAL** (per-wavelength spreading, threshold dSpread560>0.10):

- **0 регрессий** (принципиальное улучшение vs H23b scalar)
- 92/104 = 88.5% — равно baseline, не хуже
- DecorMatte→ChromataWhite: Δp95=−2.89 (6.42→3.53) — значимое улучшение
- 1930→ArtPeelBlckt: Δmed=−0.36 (2.17→1.82), Δp95=−1.45
- Все пары DecorMatte-группы: p95 снизился до 3.05–4.02, gate=3.0 не достигнут
- VibranceLuster↔RiverStone (dSpread=0.023): не затронуты — другой механизм

Вывод: per-λ spreading — правильная форма модели, нейтральный рамп даёт правильный
спектральный профиль коррекции. Остаток ошибки — в хроматических высокочернильных патчах,
где per-channel spreading (C, M, Y раздельно) отличается от нейтрального. Создан `.markdownlint.json`
для подавления pre-existing MD013/024/046/058/060 предупреждений.

---

## 2026-06-15 — H22 PASS (H22a): нейросеть k=5 = 78.1% — совпадает с D1 k=8 (3 якоря меньше)

H22 MLP (79→256→128→64→36) обучена на 23 субстратах, k-аугментация {5,8,13}, 200 эпох.
Представление входа: query [src_norm(36), cmy(3)] + mean_anchor_delta [mean(tgt-src)(36),
mean_cmy(3)] + k_norm(1) = 79 dims. Loss final: 9.1e-5 (train) / 9.9e-5 (val).

**H22a PASS**: k=5 → 89/114 = **78.1% pass** (med=0.810, P95=2.893). Совпадает с D1 k=8.
Т.е. нейросеть с **5 якорями** (paper + R + G + B + K = device-cube corners без secondaries)
достигает той же точности, что D1 с **8 якорями**. Сокращение на 3 патча.

**H22b FAIL**: k=8 → 74.6% (гейт 90% не достигнут; хуже D1 k=8=78.1%).

Структурный потолок 83.3% не пробит. Провальные 16.7% пар — те же (DecorMatte, Silverada,
VibranceMetallic) — не помогает ни больше якорей, ни нейросеть без более разнообразных
обучающих данных.

**Практический вывод**: минимальный протокол для 78.1% пар = **5 патчей**:
paper-white + R + G + B + K.

---

## 2026-06-15 — Диагностика провальных пар: WCRW 100%, DecorMatte — главный аутлайер, провалы при всех тонах

Диагностический скрипт `h_diagnose_failing_pairs.ts` выявил структуру 19 провальных пар
(k=13):

- **WatercolorRadiantWhite: 100% pass** (72/72) — субстраты внутри режима спектрально
  почти идентичны.
- **CanvasMatte 60%** — DecorMatte присутствует в 8/19 провальных парах. Matte canvas vs
  coated bright (ChromataWhite, 800M, Lyve, BelgianLinen) = фундаментально разные базовые
  спектры.
- **Ключевое наблюдение:** провальные пары дают P95 = 1.836 уже на СВЕТЛЫХ тонах (CMY
  0–1), т.е. ошибка не связана с нелинейностью при высоком покрытии — это структурное
  несовпадение спектральных форм во всём диапазоне тонов.
- **8 направленно-симметричных провалов:** пары A→B И B→A обе fail → genuinely
  incompatible substrates. 3 пары — асимметричные.
- Вывод: 16.7% потолок D1 = субстраты внутри одного Epson-режима с разными базовыми
  спектральными формами. «Один режим ≠ схожее спектральное поведение.»

→ H22 (нейросеть) должна научиться не только нелинейности по тону, но и разным базовым
спектральным отображениям для пар типа DecorMatte↔ChromataWhite.

---

## 2026-06-15 — H20 REJECTED: Kubelka-Munk в K/S пространстве катастрофически хуже D1; pre-registration H22 (neural)

**H20 (REJECTED).** Попытка заменить бумажный ratio D1 на additive ΔKS в пространстве
Kubelka-Munk (K/S=(1−R)²/(2R)). Результат: pass rate 0% при k=8 (vs 78.1% для D1),
медиана ΔE₀₀ = 7.963 vs 0.908. Причина отказа: K/S → ∞ при R→0. Тёмные якоря (R≈0.03)
дают K/S≈16; ΔKS-остатки в этих точках огромны; kNN-интерполяция разносит эти значения
на средние тона → предсказанные спектры схлопываются к нулю. Физический вывод:
двухпоточная K-M теория неприменима к глянцевым inkjet-субстратам — чернила здесь
прозрачные поглотители (не рассеиватели), что нарушает предположение мутной среды. K/S
охватывает 3–4 порядка величины по патчам, делая kNN в K-M пространстве численно
некорректным.

**Pre-registered H22 (neural).** Cross-attention spectral transfer network, обученная на
leave-one-out по 27 субстратам. Gate: k=5 → ≥78.1% pass (match D1 k=8 с 3 меньшими
якорями); k=8 → ≥90% pass. Единственный оставшийся структурный путь после провала K-M.
Реализация: Python/PyTorch + ONNX export для браузерного инференса.

Эксперимент: `EXPERIMENTS.md` строка 2026-06-15 H20. Код: `scripts/experiments/h20_km_residual.ts`.

---

## 2026-06-15 — H19: heavy-Y якоря отклонены, ранг=8 не даёт выигрыша; greedy k=8 = 78.1% pass; D-optimal хуже эвристики

H19 закрывает серию якорных экспериментов на паре DecorMatte→ChromataWhite и даёт первый ответ на вопрос «сколько патчей нужно».

**H19a (REJECTED).** Три якоря в зоне heavy-Y (130,130,0), (100,85,0), (160,170,0) увеличивают P95 с 4.787 до 5.162 (регрессия +0.375) — точно так же, как H15: добавление якорей в новый «худший сектор» вытягивает ранг-5 остаток туда и ухудшает другие сектора. Подтверждается вывод H18: структурное ограничение D1 при высокой плотности чернил нельзя исправить бо́льшим числом якорей одного класса.

**H19b (REJECTED, just misses gate).** rank=8 снижает P95 с 6.419 до 5.485 на худшей паре (ΔP95 = 0.934, гейт ≥ 1.0). Комбинация rank=8 + S1+CMY (h19bc) даёт P95 = 4.724 — лучший результат на этой паре.

**H19c (REJECTED).** Батч-запуск 114 same-mode пар: rank=5 и rank=8 дают идентичный pass rate 95/114 (83.3%). Ранг — не узкое место. (Примечание: 114 пар вместо ожидаемых 98 — `alignProfiles` после рефакторинга считает некоторые пары как валидные, которые раньше не попадали в батч.)

**D-optimal k-sweep (ключевой результат).** Greedy k=8 = **78.1% pass** (paper + 7 углов цветового пространства). D-optimal при k=8 = лишь 21.9% — намного хуже. Причина: D-optimal максимизирует объём PCA-пространства эталонных спектров, но не включает paper white и device-corner патчи, которые критичны для D1. Эвристика S1 структурно правильна. Переход от k=7→k=8 в greedy (+30.7%) объясняется: первые 7 якорей = paper + 6 углов (без чёрного), k=8 добавляет чёрный — полное покрытие device space. **Вывод для статьи:** 8 патчей (бумага + 7 хроматических углов) = 78% pass; 13 патчей = 83%.

Commits: 7b95cc2 (H19 pre-reg), dfd57b2 (h19_high_y_anchors), 5b1eedf (h19_batch_rank), 9b280c3 (h19_ksweep_dopt).
