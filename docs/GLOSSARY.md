...
| **SAMPLE_ID** | Per-patch identifier. Current format: `RGB_{R}_{G}_{B}` (e.g. `RGB_255_128_0`) — device-coordinate based, generated by `parsers/cxfParser.ts` (BC CxF3) and `parsers/cgatsParser.ts` (MOAB CGATS). Two profiles built on the same device grid share IDs → cross-profile patch matching by `alignProfiles` is exact. Previously `R{row}C{col}P{page}`; dropped because row/column indices are chart-layout-dependent and break when chart sizes differ. |
...
| **H4** | Cross-substrate transfer (k ≤ 15 anchors → ΔE00 ≤ 1.5 median, ≤ 3.0 P95, on ≥ 80 % of same-mode pairs) | **Confirmed** same-mode 80.6 % pass (2026-06-12 batch run, 98 pairs). Cross-mode: out of scope. |
...
| **H10** | CAE cross-trained on MK predicts held-out substrate from paper alone | **Rejected** in initial form (no anchor fine-tune). |
| **H10b** | Per-mode CAE_D7 + anchor fine-tune (Adam 200 steps on substrate_latent_B) | **Confirmed** for WCRW, CanvasMatte, CanvasSatin, PremiumLuster, CanvasGloss. OOD regression on USFA/Unryu (dist-to-centroid 32.9 > 25): a0 canonical for that mode. |
| **H11** | Cross-vendor same-mode device-response equivalence | **Confirmed 2026-05-29** for three overlapping presets after substrate normalisation. |
| **H12** | Single-α OBA emission scaling from anchors | **Rejected 2026-05-30** — per-profile scalar α unstable (CV 79 %); worsens median on average. |
| **H13** | Measured M0/M2 OBA correction (simple paper-scaled) | **Rejected on average** 2026-05-30 (+0.18 median). H13c adaptive gate (H13b when OBA-mismatch ≥ 0.10) gives −0.08 ΔE on OBA-disparate pairs; ships in TransferView. |
| **H14** | CAE_LOO: substrate latent search from k anchors at inference | **Conditionally confirmed** 2026-06-08 — correct algorithm; requires ≥ 3 same-mode support profiles. Fails with 1 support (PremiumLuster). WCRW 3-support: median 1.26 k=13. |
| **H15** | Per-channel cyan-ramp anchors improve Canvas Matte P95 | **Rejected** — S5 (S1 + 4 cyan-ramp) P95 equal to S1 on Canvas Matte. |
| **H16** | Per-substrate YN exponent n(λ) at 640–680 nm corrects D1 red-band error | **Rejected 2026-06-12** — null effect on P95 (Δ=0). P95 driver is OBA-mismatch in chromatic patches, not YN nonlinearity at 640–680 nm. |
