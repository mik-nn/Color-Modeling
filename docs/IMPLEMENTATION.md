...
| `lib/predict/paperRatioResidual.ts` | **D1** | Paper-relative ratio + rank-≤N PCA residual (`residualRank`, default 5). Exploits multiplicative substrate model: `R_B ≈ (R_paper_B / R_paper_A) ⊙ R_A + PCA residual` (H4). Works best on non-OBA pairs. Default rank=5 comes from H5 (median effective rank). Per-band UV clamp on 380–410 nm (`uvBandCount=4` in TransferView). The residual is interpolated **locally** per patch via IDW over anchor scores in RGB device space (`knnK`, default 4) — "Layer 3". `globalResidual` option (default false) ablates Layer 3 by giving every patch the mean anchor residual instead (global like H22); H27 shows Layer 3 is worth +12.5 pp pass-rate (88.5%→76.0%). **Limitation (H18):** error scales monotonically with total ink coverage (C+M+Y); P95 reaches 6–8 ΔE00 at maximum ink density regardless of gamut sector. |
...
| `lib/predict/batchEval.ts`         | **Batch matrix** | `evalLoadedPairs(profiles, opts)` runs the production D1 transfer (S1 k=13, OBA-separate, paper-ratio+PCA residual, OBA added back) over every ordered same-mode pair among loaded profiles. Returns per-pair median/p95/pass under the H4 gate (med≤1.5 & p95≤3.0) plus per-mode and overall pass-rate. Surfaces the aggregate "88%" the single-pair UI cannot show. RGB-only. Rendered in TransferView as a ref×tgt colour grid (green pass / rose fail, cell = p95). |
| `lib/predict/spreadCurv.ts`        | **spreadCurv** | Interpretable substrate descriptor (H36/H37): `‖c1,c2‖` of the neutral-ramp fit `R(λ)/R_paper=1+c1·a+c2·a²` at 560 nm. M0-only, robust from 3 neutral patches (white/mid/black). `computeSpreadCurv(profile)`, `classifyPairCompatibility(a,b)` (warn when `abs(Δcurv560) ≥ SPREADCURV_FAIL_THRESHOLD = 0.137`, AUC 0.84 / 83% recall), `rankReferencesByProximity(target,cands)` (nearest-curv ref halves transfer p95). NOT a corrector — H38 showed Δcurv-gated spreading fixes 0 failers; used in TransferView only as the compatibility flag + reference recommender panel. Low spreadCurv = ink holdout/on-surface (matte canvas), high = absorbed (glossy). |
