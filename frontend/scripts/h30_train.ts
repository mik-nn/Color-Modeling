// frontend/scripts/h30_train.ts
//
// H30 — Low-k retraining of H29 sharpened attention (3–5 patch protocol).
//
// The D1 sweep showed the practical battleground is k≤8 where D1 collapses
// (k=5 = 45.2%). H29 spread its k-augmentation over {5,8,13} and peaks at k=8.
// H30 retrains the SAME H29 architecture (learnable τ + hard top-k'=4 device
// gating + residual head) with low-k augmentation {3,4,5} to specialise for the
// few-patch regime — target: 5-patch pass rate > H22's 84.6%, and a usable
// 3-patch protocol where D1 is hopeless.
//
// Architecture identical to H29:
//   delta_j  = t_j - s_j
//   score_j  = (Wq·c_q)·(Wk·c_j)/sqrt(d)
//   a_j      = softmax_j(score_j / τ)  over the KPRIME nearest anchors only
...
// Loss = MSE in normalised reflectance. k-augmented over {3,4,5}.
//
// Run: cd frontend && bash -l -c "nvm use 20 && npx tsx scripts/h30_train.ts 2>&1 | tee /tmp/h30_train.log"
...
const WEIGHTS_OUT    = path.resolve(process.cwd(), 'data/cae-input/h30_weights.json')
const L              = 36
const C              = 3            // CMY device dims
const K_MAX          = 13           // fixed anchor slots (padded + masked)
const KPRIME         = 4            // hard top-k' device gating (= D1 knnK)
...
const K_TRAIN_LIST   = [3, 4, 5]      // low-k regime (H30): specialise for few patches
...
function buildSample(pd: AlignedPair, i: number, k: number): Sample {
  ...
  // Hard top-k' device gating: keep only the KPRIME anchors nearest the query
  // in device-CMY Euclidean distance (literal learned-IDW neighbourhood).
  const dist = anchors.map(ai => {
    const c = cmy(D, ai)
    return (c[0]-cq[0])**2 + (c[1]-cq[1])**2 + (c[2]-cq[2])**2
  })
  const keep = new Set(
    anchors.map((_, j) => j).sort((a, b) => dist[a] - dist[b]).slice(0, KPRIME))
...
const params = {
  ...
  logTau: tf.variable(tf.scalar(0), true, 'logTau'),  // τ = exp(logTau), init τ=1
...
function forward(
  ...
  return tf.tidy(() => {
    ...
    // score[B,K] = Σ_d q·k / (sqrt(d)·τ), τ = exp(logTau) learnable temperature
    const tau = tf.exp(params.logTau)
    const score = tf.sum(tf.mul(kk, q.reshape([B, 1, ATTN_DIM])), 2)
      .div(Math.sqrt(ATTN_DIM)).div(tau)                       // [B,K]
    // masked softmax: pad + non-top-k' anchors set to -1e9 before softmax
...
async function main() {
  ...
  await evalPairs(valPairs, [3, 4, 5, 8], 'Validation (held-out substrates):')
  const allPass = await evalPairs(allPairs, [3, 4, 5, 8], 'All same-mode BC pairs:', d1only)

  const tauLearned = Math.exp((await params.logTau.data())[0])
  console.log(`\n  Learned temperature τ = ${tauLearned.toFixed(4)} (τ→0 = hard nearest, 1 = H28 init)`)
  console.log('  Reference — H22 k=5: 84.6%  |  H29 k=5: 82.7%  |  D1 k=5: 45.2%  |  D1 k=8: 82.7%')
  console.log('  Gate H30a: k=5 > 84.6% (beat H22)   Gate H30b: k=3 ≥ 70% (usable 3-patch)')
  ...
  console.log(`  Recovered ${recovered}/3 at k=5 (info; D1 needs k=13 for these)`)
  ...
  await fs.writeFile(WEIGHTS_OUT, JSON.stringify({
    arch: 'h30_lowk_sharp_attention', L, C, K_MAX, KPRIME, ATTN_DIM, HEAD_HID, ALPHA,
    tau: tauLearned, k_list: K_TRAIN_LIST, weights: wData,
  }))
