// frontend/scripts/h29_train.ts
//
// H29 — Sharpened device attention = literal learned IDW.
//
// H28 (per-anchor softmax attention) had the best median of any method but
// FAILED the P95 gate: softmax/sqrt(d) is too diffuse, averaging over many
// anchors instead of D1's "nearest dominates" (H27 Layer-3 IDW). H29 sharpens
// it two ways on top of H28:
//   (1) learnable temperature τ:  a_j = softmax(score_j / τ),  τ = exp(logτ)
//       → τ→0 = hard nearest-anchor, τ large = diffuse (H28).
//   (2) hard top-k' device gating: mask all but the KPRIME=4 anchors nearest
//       the query in device-CMY Euclidean distance, BEFORE softmax. Attention
//       runs over those 4 only — the literal learned analogue of D1 IDW (k=4).
//
// Per query patch q (normalised src spectrum s_q∈R36, device CMY c_q∈R3):
//   delta_j  = t_j - s_j
...
//
// Run: cd frontend && bash -l -c "nvm use 20 && npx tsx scripts/h29_train.ts 2>&1 | tee /tmp/h29_train.log"
...
const WEIGHTS_OUT    = path.resolve(process.cwd(), 'data/cae-input/h29_weights.json')
const L              = 36
...
async function main() {
  ...
  console.log('  Reference — H22 k=5 S1: 84.6%  |  D1 k=13: 88.5%  |  H28 k=13: 84.6% (med 0.735)')
  console.log('  Gate H29a: all-pairs k=5 ≥ 84.6%   Gate H29b: recover ≥2/3 H27 D1-only pairs')
  ...
  console.log(`  Recovered ${recovered}/3 → Gate H29b ${recovered >= 2 ? 'PASS' : 'FAIL'}`)
  ...
  await fs.writeFile(WEIGHTS_OUT, JSON.stringify({
    arch: 'h29_sharp_attention', L, C, K_MAX, KPRIME, ATTN_DIM, HEAD_HID, ALPHA,
