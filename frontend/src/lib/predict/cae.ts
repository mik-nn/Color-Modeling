...
class CAEForward {
  ...
  encodeSubstrate(paper: ArrayLike<number>, subId: number): Float64Array {
    ...
    matVec(
      this.w['substrate_fc2.weight'],
      this.subHidden,
      this.w['substrate_fc2.bias'],
      this.subOut,
    )
...
export function runCAETransfer(input: CAERunInput): CAERunResult {
  ...
  if (input.anchorResiduals && input.anchorResiduals.length > 0) {
    anchorFineTuned = true
    ...
    for (const res of input.anchorResiduals) {
      for (let i = 0; i < weights.arch.substrate_latent_dim; i++) {
        avgSubResidual[i] += res.subB[i] - res.subA[i]
      }
      for (let i = 0; i < weights.arch.ink_latent_dim; i++) {
        avgInkResidual[i] += res.inkLat_B[i] - res.inkLat_A[i]
      }
    }

    // Normalize by number of anchors
    const nAnchors = input.anchorResiduals.length
    for (let i = 0; i < weights.arch.substrate_latent_dim; i++) {
      avgSubResidual[i] /= nAnchors
    }
    for (let i = 0; i < weights.arch.ink_latent_dim; i++) {
      avgInkResidual[i] /= nAnchors
    }
    ...
    for (let i = 0; i < weights.arch.ink_latent_dim; i++) {
      // We'll keep ink latent as is for now to avoid overcorrection
      // subB[i] += learningRate * avgInkResidual[i]; // Commented out for stability
    }
  }
  ...
  const report = evaluatePrediction({
    variant: `CAE_${weights.variant.toUpperCase()}${refInTrain ? '' : '_heldRef'}${targetInTrain ? '' : '_heldTgt'}${anchorFineTuned ? '_ft' : ''}`,
    k: anchorIdx.length,
    XPred: XPredTest,
    XTrue: XTrueTest,
    L,
