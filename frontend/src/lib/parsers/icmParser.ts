 ...
 */
async function extractMeasurementsFromProfile(dataView: DataView): Promise<Measurement[]> {
 ...
 */
function parseMatrixLookupTable(dataView: DataView, offset: number): Measurement[] {
  const measurements: Measurement[] = []
  ...
  for (let i = 0; i < Math.min(totalPoints, 500); i++) {
    ...
    if (isCMYK) {
      ...
      c =
        ((Math.floor(temp / (gridPoints * gridPoints * gridPoints)) % gridPoints) /
          (gridPoints - 1)) *
        100
    } else {
      // RGB
      ...
      const r =
        ((Math.floor(temp / (gridPoints * gridPoints)) % gridPoints) / (gridPoints - 1)) * 100
      c = 100 - r
      m = 100 - g
      y = 100 - b
    }
 ...
 */
function parseCLUT(dataView: DataView, offset: number): Measurement[] {
  const measurements: Measurement[] = []
...
function cmykToLabApprox(
  ...
  const Y = rl * 0.2126729 + gl * 0.7151522 + bll * 0.072175
  const Z = rl * 0.0193339 + gl * 0.119192 + bll * 0.9503041
  ...
  const fx = X / Xn <= 0.008856 ? (X / Xn) * 7.787 + 16 / 116 : Math.pow(X / Xn, 1 / 3)
  const fy = Y / Yn <= 0.008856 ? (Y / Yn) * 7.787 + 16 / 116 : Math.pow(Y / Yn, 1 / 3)
  const fz = Z / Zn <= 0.008856 ? (Z / Zn) * 7.787 + 16 / 116 : Math.pow(Z / Zn, 1 / 3)
